...
| Database schema/migrations | Versioned migrations; generated TS types | `supabase/migrations/` (19 migrations through `20260905000000_beta_review_evidence.sql`, including `20260902170123_family_invitation_provisioning.sql`, `20260902171500_family_invitation_anon_grant_hardening.sql`, and `20260902174500_family_invitation_terminal_state_guard.sql`; each has a `-- rollback:` block); types at `src/lib/supabase/database.types.ts`, generated from the linked project by `npm run db:types` and drift-verified by `npm run db:types:check`. |
...
| External checkout | Program-specific URL handoff and explicit pending state | `programs.checkout_url` column + `src/components/program/checkout-handoff.tsx`. Since Slice 4 (2026-09-19, MTS-CHG-015), 8 of the 9 published programs carry the exact GoDaddy checkout their button opens on the approved classes page. Tutoring is NULL. The allowed forms are `https://poynt.godaddy.com/checkout/2bf1b322-d362-4d5d-a4a7-5e5791473f14/<id>` (Home School Haven's business path only) and `https://pay.homeschoolhaven.org/…`, with no query or fragment. They are enforced by `private.is_approved_checkout_url` in `admin_update_program_facts` and by the `programs_checkout_url_approved` CHECK. A link renders only on a family's own `started` registration, never on the public page. No provider signal is consumed. Payment is verified manually, by the audited administrator transition to `confirmed`. |
